...
import { idPathSlug, resolveContainedCapturePath, toolPathSlug } from './path-safety.js'
...
export function buildFilePath(walletDir: string, conversation: CapturedConversation): string {
  ...
  const shortId = idPathSlug(conversation.id.slice(-6))
  const filename = `${toolPathSlug(conversation.tool)}-${time}-${shortId}.md`
  return resolveContainedCapturePath(walletDir, 'conversations', date, filename)
}
